...
function changeToBlack() {
  sayHelloButton.classList.add("changeToBlack");
}
...
function changeBack() {
  sayHelloButton.classList.remove("changeToBlack");
}
...
const repeatMyParam = () => {
  axios.get(`http://localhost:3000/repeat/:Kyson}`).then(function (response) {
...
function attach() {
  axios
    .get("http://localhost:3000/query-test?name=Kyson")
    .then((res) => {
      console.log(res);
    })
    .catch((err) => {
...
document.querySelector("#query-button").addEventListener("click", attach);
...
/*
    ...
    // It said I sent an empty query

    2: Send more than 1 query on the URL -- what happened? 

    // It told me I sent more then 1 query and the querys appeared under my data tab
*/
...
function createFood(event) {
  ...
  axios
    ...
    .then(function (res) {
      ...
      newF.textContent = res.data;
      document.querySelector("body").appendChild(newF);
    })
